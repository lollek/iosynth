...
var (
	...
	voice2soundWave = map[byte]func(float64) float64{
		0: sound.SineWaveFn,
		1: sound.SawtoothWaveFn,
	}
)

func parseCommand(command string) {
	// command = [VOICE] [OCTAVE] [NOTE] [Optional] [Optional]
	if len(command) < 3 || len(command) > 5 {
		return
	}

	waveFn := voice2soundWave[command[0] - '0']
	if waveFn == nil {
		return
	}
	...
	sound := sound.NewSoundWave(freq, duration, waveFn)
